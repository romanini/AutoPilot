...
void print_gps() {
  ...
  DEBUG_PRINTLN("RAW GPS::");
  if (date_lost) {
    DEBUG_PRINT(" lost: [");
    DEBUG_PRINT(date_lost_time - date_obtained_time);
    DEBUG_PRINT("] ");
  }
  DEBUG_PRINT(gps.year);
  DEBUG_PRINT("/");
  DEBUG_PRINT(gps.month);
  DEBUG_PRINT("/");
  DEBUG_PRINT(gps.day);
  DEBUG_PRINT(" ");
  DEBUG_PRINT(gps.hour);
  DEBUG_PRINT(":");
  DEBUG_PRINT(gps.minute);
  DEBUG_PRINT(":");
  DEBUG_PRINTLN(gps.seconds);
  DEBUG_PRINT(gps.fix);
  DEBUG_PRINT(" / ");
  DEBUG_PRINT(gps.fixquality);
  DEBUG_PRINT(" / ");
  DEBUG_PRINT(gps.fixquality_3d);
  DEBUG_PRINT(" / ");
  DEBUG_PRINTLN(gps.satellites);
  DEBUG_PRINT(gps.speed);
  DEBUG_PRINT(" / ");
  DEBUG_PRINT2(gps.latitudeDegrees, 14);
  DEBUG_PRINT(" / ");
  DEBUG_PRINT2(gps.longitudeDegrees, 14);
  DEBUG_PRINT(" / ");
