#!/usr/bin/env python3
# Bridges the wind fields carried on the controller's ~APDAT broadcast (UDP
# 8888, see firmware/Arduino/controller/publish.ino) into standard NMEA0183
# MWV sentences sent over UDP. This lets OpenCPN's stock Dashboard plugin
# display wind on its own gauge with no new plugin code - see the "Wind
# display in OpenCPN" section of navigator/README.md for how to point an
# OpenCPN connection at this.
#
# Cadence note: this can only update as fast as ~APDAT is broadcast (1 Hz by
# default), even though the wind board itself samples at 5 Hz - the
# controller only merges and re-broadcasts once per PUBLISH_INTERVAL. Raise
# that in publish.ino if the gauge feels sluggish; nothing here needs to
...
# The wind fields are trailing/optional, same convention every ~APDAT
# consumer in this project follows - a frame without them is just skipped,
# not an error, so a display running against older controller firmware
# doesn't spam the log.
REQUIRED = ("awa", "aws_kn", "wind_ok", "twa", "tws_kn", "true_wind_ok")
...
def main():
    parser = argparse.ArgumentParser(
        description="Bridge ~APDAT wind fields to NMEA0183 MWV sentences over UDP, "
        "so OpenCPN's Dashboard plugin can display wind with no new plugin code."
    )
    ...
    while True:
        ...
        if not all(field in values for field in REQUIRED):
            if not warned_missing_fields:
                print("~APDAT frame is missing wind fields - is the controller firmware old?", file=sys.stderr)
                warned_missing_fields = True
            continue
        for sentence in build_sentences(values):
