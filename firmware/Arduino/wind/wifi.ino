...
// The wind sensor lives headless at the top of the mast - the least reachable
// place on the boat - so it must never need a power cycle to rejoin the AP.
// Two cases are handled: SoberPilot not up yet at boot (the whole boat just got
// power and the controller is still coming up), and the link dropping later.
// See setup_wifi() and check_wifi() below. Identical in shape to
// rudder/wifi.ino, for identical reasons.
...
#define WIFI_SETUP_ATTEMPTS 3         // boot-time attempts before deferring to command_task
...
// on its own task, so a relayed ~APCMD,v$ is still serviced during this wait.
...
void setup_wifi() {
  ...
  // latency to the telemetry stream. The board is 12 V powered off the
  // masthead light circuit, so the extra draw doesn't matter.
  ...
  if (!wifiWasConnected) {
    ...
    // listener never started, needing a trip up the mast (or at least a
    // masthead breaker cycle) once SoberPilot came up.
    DEBUG_PRINTLN("No SoberPilot at startup - command_task will keep retrying");
  }
}

// Polled from command_task (wind.ino) - deliberately not from the sampling
// task, since try_connect_wifi() above blocks for up to
...
// would publish fine but silently stop accepting ~APCMD,v$ (same reason
// rudder/wifi.ino and display/subscribe.ino re-call setup_subscribe() on their
// reconnect paths).
void check_wifi() {
  if (WiFi.status() == WL_CONNECTED) {
    if (!wifiWasConnected) {
